                ...
                <div class="bg-black/20 p-6 rounded-2xl border border-white/5 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <AppInput v-model="form.frequency" :label="$t('schedules.frequency')" type="select" required>
                            <option value="Hourly">{{ $t('schedules.freq_every_hour') }}</option>
                            ...
                            <option value="Quarterly">{{ $t('schedules.freq_3_months') }}</option>
                            <option value="Semiannually">{{ $t('schedules.freq_6_months') }}</option>
                            <option value="Yearly">{{ $t('common.yearly') || 'Yearly' }}</option>
                            <option value="CustomHours">{{ $t('schedules.freq_custom_hours', { interval: 'X' }) }}</option>
                        </AppInput>
...
const loadData = async () => {
    ...
    try {
        ...
        toast.error(t('schedules.load_failed') || 'Failed to load schedule data.');
    } finally {
        loading.value = false;
    }
};
